const ApiKey = "6edf2aabd96d4935a766d5ac6635c1c2";
...
function getListTeams() {
    ...
    fetch(teamEndPoin, fetchHeader)
        ...
        .then(resJson => {
            ...
            const desc = document.querySelectorAll('.secondary-content')
            desc.forEach(btn=>{
                btn.onclick=(event)=>{
...
function showTeamInfo(id){
    title.innerHTML = "INFORMASI TEAM";
    fetch(nameClub+id, fetchHeader)
        .then(response => response.json())
        .then(resJson => {
            // console.log(resJson.name);
            let logo = resJson.crestUrl
            let clube = resJson.name
            let clube2 = resJson.founded
            let clube3 = resJson.venue
            let clube4 = resJson.shortName
            let clube5 = resJson.address
            let club6 = resJson.activeCompetitions[0].name;
            contents.innerHTML = 
            "<img src='"+logo+"'>"+"<br>Nama Club : "+ clube + 
            "<br>"+ "Didirikan : "+ clube2 + 
            "<br>"+"Stadion : "+ clube3 +
            "<br>"+"Alias : "+ clube4 +
            "<br>"+"Alamat Stadion : "+ clube5
            + "<br> League : "+ club6;
